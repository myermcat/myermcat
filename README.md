<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:12c2e9,50:c471ed,100:f64f59&height=200&section=header&text=hi%2C%20I'm%20Mariia%20%F0%9F%91%8B&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=I%20break%20things%20to%20understand%20them%2C%20and%20build%20them%20better&descSize=18&descAlignY=60" width="100%" alt="hi, I'm Mariia" />

<img src="https://readme-typing-svg.demolab.com/?lines=computer+engineering+%40+uOttawa;founder+%26+chair%2C+CE+Students+Society;I+ship+things+with+AI+agents+%F0%9F%A4%96;certified+cat+person+%F0%9F%90%88;I+used+to+study+stars.+literally.&font=Fira+Code&center=true&width=440&height=45&color=c471ed&vCenter=true&pause=1200&size=18" alt="typing intro" />

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-let's_connect-4D96FF?style=for-the-badge&logo=data:image/svg%2bxml;base64,)](https://www.linkedin.com/in/myerm)
[![Resume](https://img.shields.io/badge/Resume-the_formal_version-FF6B9D?style=for-the-badge&logo=readdotcv&logoColor=white)](https://myermcat.github.io/myermcat/Mariia_Yermolenko_Resume.pdf)
[![The Digital Lifecycle Guide](https://img.shields.io/badge/Digital_Lifecycle_Guide-see_it_live-6BCB77?style=for-the-badge&logo=googlechrome&logoColor=white)](https://myermcat.github.io/digital-lifecycle-guide/)

</div>

## 🌈 about me

- 🎓 **Computer Engineering @ University of Ottawa** (co-op, Dean's Honours List)
- 🏛️ built **[The Digital Lifecycle Guide](https://myermcat.github.io/digital-lifecycle-guide/)** at the **Treasury Board of Canada Secretariat** — 30+ pages on how to run a government digital service from birth to retirement, shipped with Cursor, Claude Code, and stubbornness
- 🧑‍🤝‍🧑 **founded the Computer Engineering Students Society** at uOttawa and ran a 200+ attendee TED-style conference from scratch
- 🤖 co-organized **Canada's largest AI hackathon** (400+ humans, uncounted agents) with AGI Ventures
- 🐍 once automated **500+ hours a year** of manual work out of existence with 23 Python scripts
- 🔭 in a previous life: **150+ astrophysics Wikipedia articles** and published research on variable stars. the stars were also debugged.
- 🐈 cats. always cats.

## 🧰 toolbox

<div align="center">

![Python](https://img.shields.io/badge/Python-4D96FF?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-f64f59?style=for-the-badge&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-9D4EDD?style=for-the-badge&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-FFD93D?style=for-the-badge&logo=javascript&logoColor=black)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-FF6B9D?style=for-the-badge&logo=html5&logoColor=white)

![Android Studio](https://img.shields.io/badge/Android_Studio-6BCB77?style=for-the-badge&logo=androidstudio&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFA62B?style=for-the-badge&logo=firebase&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-12c2e9?style=for-the-badge&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git_%26_GitHub-c471ed?style=for-the-badge&logo=git&logoColor=white)

![Claude Code](https://img.shields.io/badge/Claude_Code-my_pair_programmer-c471ed?style=flat-square&logo=anthropic&logoColor=white)
![Cursor](https://img.shields.io/badge/Cursor-yes_that_too-FFD93D?style=flat-square&logo=databricks&logoColor=black)
![Coffee](https://img.shields.io/badge/fueled_by-deadlines_%26_cats-f64f59?style=flat-square&logo=buymeacoffee&logoColor=white)

</div>

## 🛠️ things I've built

| | project | what it is |
|---|---|---|
| 🏛️ | **[digital-lifecycle-guide](https://github.com/myermcat/digital-lifecycle-guide)** | a guide for running Government of Canada digital services — the whole life, Create → Live → Sunset |
| 📱 | **[LocalLoop](https://github.com/farahes/SEG2105_localLoopProject_Group6)** | Android event app; led the dev team; named **Best App of the 2025 cohort** 🏆 |
| 🎧 | **[whisper-ai-transcriber](https://github.com/myermcat/whisper-ai-transcriber)** | Python + Whisper: audio in, text out, meetings survived |
| 📊 | **[BusinessVenueAnalyzer](https://github.com/myermcat/BusinessVenueAnalyzer)** | hackathon build that sizes up business venues. self-described: *"the best analyzer"* |
| 🎬 | **[get-youtube-captions-with-timestamps](https://github.com/myermcat/get-youtube-captions-with-timestamps)** | tiny tool that rips YouTube transcripts, timestamps included |

## 🎲 currently

- 🔧 breaking things at uOttawa to understand them (it's called *engineering*)
- 🏗️ growing the CE Students Society into a thing that outlives me
- 🗺️ figuring out what governments, hackathons, and cats have in common: they all need good infrastructure

<div align="center">

<img src="https://readme-jokes.vercel.app/api?theme=tokyonight&hideBorder" alt="a random dev joke, refreshed on every visit" />

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:f64f59,50:c471ed,100:12c2e9&height=120&section=footer" width="100%" alt="" />

</div>
